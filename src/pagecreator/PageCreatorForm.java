...
public class PageCreatorForm extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        genreButton = new javax.swing.JMenuItem();
        mpaaButton = new javax.swing.JMenuItem();
        ...
        jMenu3.add(genreButton);

        mpaaButton.setText("MPAA");
        mpaaButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mpaaButtonActionPerformed(evt);
            }
        });
        jMenu3.add(mpaaButton);
...
/*
    ...
    */
    ...
    private void genreButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_genreButtonActionPerformed
        ...
        for (int rows = 1; rows < tableRows; rows ++ ){
            if (!this.jTable1.getValueAt(rows, tableColumn).equals("Adult")){
                jTable1.setValueAt("Adult", rows, tableColumn);
                this.changedMov(rows, tableColumn);
            }
        }
    }//GEN-LAST:event_genreButtonActionPerformed

    private void mpaaButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mpaaButtonActionPerformed
        int tableColumn = 5;
        int tableRows = jTable1.getRowCount();
        for (int rows = 1; rows < tableRows; rows ++ ){
            if (!this.jTable1.getValueAt(rows, tableColumn).equals("R")){
                jTable1.setValueAt("R", rows, tableColumn);
                this.changedMov(rows, tableColumn);
            }
        }
    }//GEN-LAST:event_mpaaButtonActionPerformed
    ...
    private void changedMov(int row, int column){
        ...
        if (changedMovies.size() == 0) {
         ...
         //publisher
                if (column == 1){
                  xmlChanged.setProducer(jTable1.getValueAt(row, column).toString()); 
                }
                //name
                if (column == 2){
                    xmlChanged.setTitle(jTable1.getValueAt(row, column).toString());
                }
                //sortname
                if (column == 3){
                    xmlChanged.setSorttitle(jTable1.getValueAt(row, column).toString());
                }
                //Genre
                if (column == 4){
                    xmlChanged.setGenre(jTable1.getValueAt(row, column).toString());
                }
                //mpaa
                if (column == 5){
                    xmlChanged.setMpaa(jTable1.getValueAt(row, column).toString());
                }
                //runtime
                if (column == 6){
                   xmlChanged.setRuntime(jTable1.getValueAt(row, column).toString());
                }
                //year
                if (column == 7){
                    xmlChanged.setYear(jTable1.getValueAt(row, column).toString());
                }
                //plot
                if (column == 8){
                    xmlChanged.setPlot(jTable1.getValueAt(row, column).toString());
                }
                //language
                if (column == 9){
                   xmlChanged.setLanguage(jTable1.getValueAt(row, column).toString());
                }
        ...
        for(String[] movie: movies) {
            for(String mov: movie){
                ...
                if (Moviename.getInstance().FileExist(lookups.folder+"\\folder.jpg")){
                    ...
                    //lookups.id = counter;
                    lookups.resizePicName = this.cleanResizePicName(publisher[publisherCount] + "_" + mov + ".jpg");
   ...
   private String cleanResizePicName(String resizePicName){
    ...
    private javax.swing.JMenuItem genreButton;
    ...
    private javax.swing.JMenuItem mpaaButton;
