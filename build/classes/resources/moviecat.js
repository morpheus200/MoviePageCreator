...
function active_sort(id, dir)
{
    ...
    if (sort_dir)
        sort_dir.parentNode.removeChild(sort_dir);
...
function sort_producer()
{
    do_sort("PRODUCER", "text", +1);
}
...
/*
function mpaa_set_all(x)
{
    var gtable = document.getElementById("MPAA_TABLE");
    var gbox = gtable.getElementsByTagName("input");
    var i;
    for (i = 0; i < gbox.length; i++) {
        if (gbox[i].type === "checkbox") {
...
/*function mpaa_one(x)
{
    if (!initialized)
        return;
    debug(x);
    var mpbox = document.getElementById(x);
    if (mpbox.type === "checkbox") {
        mpaa_set_all(false);
        mpbox.checked = true;
        do_filter();
    }
...
/*
var mpaamatch_mode = 0;

function mpaa_get()
{
    var mptable = document.getElementById("MPAA_TABLE");
    var mpbox = mptable.getElementsByTagName("input");
    var mpaa = new Array();
    var i;
    var all_g = 0;
    for (i = 0; i < mpbox.length; i++) {
        if (mpbox[i].type !== "checkbox")
            continue;
        all_g++;
        if (mpbox[i].checked) {
            //genre.push(gbox[i].id.substr(2)); // strip G_
            mpaa.push(mpbox[i].id.substr(2));
        }
    }
    if (mpaa.length > all_g / 2) {
        // more than half genres selected
        // match all in movie mode
        mpaamatch_mode = 1;
    } else {
...
/*function mpaa_match(mpstring, mparray)
{
    //mpstring = mpstring.replace(/[ \/]+/g, " ").replace(/[()]/g, "");
    var mgarr = mpstring ? mpstring.split(" ") : [];
    var i;
    if (mpaamatch_mode === 0) {
        // match any
        for (i = 0; i < mgarr.length; i++) {
            if (indexOf(mparray, mgarr[i]) !== -1)
                return true;
        }
        return false;
    } else {
...
{
    var table = document.getElementById("TAG_TABLE");
   //var table = document.getElementById("MPAA_TABLE");
    var input = table.getElementsByTagName("input");
    var tags = new Array();
    var i;
    debug_add("<br>Tags: ");
...
function tag_match(tagstr, tagarray)
{
    ...
    for (i = 0; i < tagarray.length; i++) {
        ...
        if (tagarray[i].value === "set" && !present)
            return false;
        if (tagarray[i].value === "not" && present)
            return false;
    }
    return true;
}
  ...
  //  rmax = document.getElementById("RMAX").value;
   ...
   // debug_add("<br>Ranges: Y:" + ymin + "-" + ymax + " R:" + rmin + "-" + rmax + " T:" + tmin + "-" + tmax);
    ...
    for (i = 0; i < rows.length; i++) {
        ...
        //mpaas = getValue(rows[i], "MMPAA");
        //show = mpaa_match(mpaas, mpaa);
     ...
     //   if (mr < rmin || mr > rmax)
      //      show = false;
        ...
        if (mrt < tmin || mrt > tmax)
            show = false;
       // mtags = getValue(rows[i], "MTAGS");
       mtags = getValue(rows[i], "MMPAA");
        if (!tag_match(mtags, tags))
            show = false;
        if (show) {
            //debug_add(i + mg + "<br>");
